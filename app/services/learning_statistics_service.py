...
class LearningStatisticsService:
    ...
    def get_learning_summary(db: Session) -> Dict[str, Any]:
        ...
        return LearningStatisticsService._get_cached_or_fetch(
            cache_key,
            LearningStatisticsService._fetch_learning_summary,
            db
        )
    ...
    def _fetch_learning_summary(db: Session) -> Dict[str, Any]:
        ...
        avg_score = db.query(func.avg(EventoEstado.puntuacion)).filter(
            and_(
        ...
        evaluated_count = db.query(EventoEstado).filter(
            and_(
                EventoEstado.puntuacion.isnot(None),
                EventoEstado.estado == "completado"
            )
        ...
        total_with_score = db.query(EventoEstado).filter(
            and_(
                EventoEstado.puntuacion.isnot(None),
                EventoEstado.estado == "completado"
            )
        ...
        passed_count = db.query(EventoEstado).filter(
            and_(
                EventoEstado.puntuacion >= 5,
                EventoEstado.estado == "completado"
            )
        ).count()
        ...
        avg_time = db.query(func.avg(EventoEstado.duracion)).filter(
            and_(
    ...
    def get_average_score_by_activity(db: Session) -> Dict[str, List]:
        ...
        return LearningStatisticsService._get_cached_or_fetch(
            cache_key,
            LearningStatisticsService._fetch_average_score_by_activity,
            db
        )
    ...
    def _fetch_average_score_by_activity(db: Session) -> Dict[str, List]:
        ...
        results = db.query(
            Actividad.nombre,
            func.avg(EventoEstado.puntuacion).label('avg_score')
        ).join(
            EventoEstado, EventoEstado.id_actividad == Actividad.id
        ).filter(
            and_(
                EventoEstado.puntuacion.isnot(None),
                EventoEstado.estado == "completado"
            ...
            func.avg(EventoEstado.puntuacion).desc()
        ).all()
    ...
    def get_score_distribution(db: Session) -> Dict[str, List]:
        ...
        return LearningStatisticsService._get_cached_or_fetch(
            cache_key,
            LearningStatisticsService._fetch_score_distribution,
            db
        )
    ...
    def _fetch_score_distribution(db: Session) -> Dict[str, Any]:
        ...
        scores_query = db.query(EventoEstado.puntuacion).filter(
            and_(
                EventoEstado.puntuacion.isnot(None),
                EventoEstado.estado == "completado"
            )
    ...
    def get_time_boxplot_by_activity(db: Session) -> Dict[str, Any]:
        ...
        return LearningStatisticsService._get_cached_or_fetch(
            cache_key,
            LearningStatisticsService._fetch_time_boxplot_by_activity,
            db
        )
    ...
    def _fetch_time_boxplot_by_activity(db: Session) -> Dict[str, Any]:
        ...
        for activity in activities:
            ...
            times_query = db.query(EventoEstado.duracion).filter(
                and_(
